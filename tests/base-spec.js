...
describe("Base Test Suite", () => {
  ...
  it("can parse unicode text and emoji", () => {
    const ast = parse(`
      <view>中文内容🙂</view>
    `);
    const matches = esquery(ast, "WXText");
    expect(matches.some((node) => node.value === "中文内容🙂")).to.be.true;
    expect(ast.errors).to.be.lengthOf(0);
  })

  it("can parse unicode attribute value and interpolation", () => {
    const ast = parse(`
      <view title="你好{{name}}🙂" />
    `);
    const attrMatches = esquery(ast, "WXAttribute");
    expect(attrMatches).to.be.lengthOf(1);
    expect(attrMatches[0].value).to.be.equals("你好{{name}}🙂");
    expect(attrMatches[0].rawValue).to.be.equals("\"你好{{name}}🙂\"");
    const intpMatches = esquery(ast, "WXAttributeInterpolation");
    expect(intpMatches).to.be.lengthOf(1);
    expect(intpMatches[0].value).to.be.equals("name");
  })

  it("can parse unicode comment", () => {
    const ast = parse(`
      <!-- 中文🙂 -->
    `);
    const matches = esquery(ast, "WXComment");
    expect(matches).to.be.lengthOf(1);
    expect(matches[0].value).to.be.equals(" 中文🙂 ");
  })
